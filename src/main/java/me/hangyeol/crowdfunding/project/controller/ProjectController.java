...
public class ProjectController {
    ...
    @GetMapping("")
    public ResponseEntity<List<ProjectDto.InfoResponse>> readAll(@RequestParam(value = "pageNum", defaultValue = "1") String pageNum) {
        return ResponseEntity.ok().body(projectService.readAll(Integer.parseInt(pageNum)));
    }

    @ApiOperation(value = "프로젝트 등록, 날짜 입력 포맷 : 2020-01-01T13:00")
    @PostMapping("") // Create, Delete 는 ResultMessage를 담는 객체를 만들어서 리턴
    public ResponseEntity<ProjectDto.InfoResponse> create(UserDto.InfoRequest userDto, ProjectDto.CreateRequest projectDto) {
        ProjectDto.InfoResponse project = projectService.create(userDto, projectDto);
        return ResponseEntity.status(HttpStatus.CREATED).body(project);
    }

    @ApiOperation(value = "프로젝트 정보 확인")
    @GetMapping("/{title}")
    public ResponseEntity<ProjectDto.InfoResponse> readDetail(@PathVariable String title) {
    ...
    public ResponseEntity<ProjectDto.InfoResponse> update(@PathVariable String title, UserDto.InfoRequest user, ProjectDto.UpdateRequest projectDto) {
        User userDto = projectService.findByEmail(user.getEmail());
        ProjectDto.InfoResponse project = projectService.update(title, userDto.toUserDto(), projectDto);
    ...
    public ResponseEntity<FundingDto.totalRequest> fundingResult(ProjectDto.InfoResponse projectDto) {
